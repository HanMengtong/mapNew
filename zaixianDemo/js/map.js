	...
	var SVGMap = (function () {
		...
		SVGMap.prototype.render = function () {
			...
			var offsetXY = function (obj) {
				var mouseX = $(obj.node).offset().left+$(obj.node).outerWidth()/2, mouseY = $(obj.node).offset().top+$(obj.node).outerHeight()/2;
			...
			for (var state in mapConfig.shapes) {
				...
				var font = r.text(offsetX,offsetY,obj.name).attr({cursor: 'pointer','metadata' : {mapId: 1 },'font-size':mapConfig['names'][state]['font-size'],'font-family':mapConfig['names'][state]['font-family'],'fill':mapConfig['names'][state]['font-color']});
				obj.font = font;
				font.obj = obj;
